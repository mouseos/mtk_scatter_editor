...
if len(sys.argv) < 2:
	print("Usage: python mtk_scatter_editor.py Scatter Output")
	sys.exit(1)
...
try:
	...
	cnt=0
	table=[]
	head=["Part number","Name","Start(B)","End(B)","Size(B)","Size(optimized)"]
	for data in scatter:
	...
	while(True):
		...
		if(part_num in range(len(table))):
			...
			print("Please select exist partition")
	print("Expand:1\nShrink:2")
	print("Select：", end="")
	while(True):
		option=int(input())
	...
	if(option ==1 or option==2):
		while(True):
			print("Please enter size(Ex：120 100K 10M 2G)")
			print("Size：", end="")
			size=input()
		...
		for i in range(part_num,len(scatter)):
			print("Processing "+scatter[i]["partition_name"]+"...")
			if(i==part_num):
				if(option==1):
